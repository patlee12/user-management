...
export class AuthController {
  ...
  async googleAuthCallback(
    ...
    // Grab the original redirect or fall back to '/'
    const raw = Array.isArray(req.query.redirect)
      ? req.query.redirect[0]
      : req.query.redirect;
    const redirect = raw ? raw.toString() : '/';

    // Finally, send the browser back to the frontend login page to continue login process
    return res.redirect(
